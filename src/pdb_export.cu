...
void export_particles_to_pdb(particle_box_t const &box,
  ...
  for (size_t p_idx = 0; p_idx < box.particle_count; p_idx++) {
    ...
    //   file << "PATCH" << std::right << std::setw(6) << std::fixed << (p_idx + 1)
    //       << "  P   NONE   1" << std::right << std::setw(12) << std::fixed
    //       << patch.pos.y*patch.radius + box.particles[p_idx].pos.x << std::right << std::setw(8)
    //       << patch.pos.z*patch.radius + box.particles[p_idx].pos.y << std::right << std::setw(8)
    //       << patch.pos.w*patch.radius + box.particles[p_idx].pos.z << std::right << std::setw(8)
    //       << "  1.00  1.00" << std::endl;
    // }
  }
  ...
  for (size_t p_idx = 0; p_idx < box.particle_count; p_idx++) {
    for (size_t i = 0; i < box.particles[p_idx].patch_count; i++) {
      ...
      file << "ATOM" << std::right << std::setw(7) << std::fixed
           << (atom_index_offset + i + 1) 
           << "  P   NONE   1" << std::right << std::setw(12) << std::fixed
           << patch.pos.y*box.particles[p_idx].radius + box.particles[p_idx].pos.x << std::right << std::setw(8)
           << patch.pos.z*box.particles[p_idx].radius + box.particles[p_idx].pos.y << std::right << std::setw(8)
           << patch.pos.w*box.particles[p_idx].radius + box.particles[p_idx].pos.z << std::right << std::setw(8)
           << "  1.00  1.00" << std::endl;
    }
